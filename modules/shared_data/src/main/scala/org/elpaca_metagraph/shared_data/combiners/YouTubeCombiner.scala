...
object YouTubeCombiner {
  private def isNewDay(data: YouTubeRewardInfo, currentEpochProgress: EpochProgress): Boolean =
    data.epochProgressToReward.value.value + epochProgressOneDay < currentEpochProgress.value.value

  def updateYouTubeState(currentCalculatedState: Map[DataSourceType, DataSource],
                         currentEpochProgress: EpochProgress,
                         youTubeUpdate: YouTubeUpdate,
                         applicationConfig: ApplicationConfig): YouTubeDataSource = {
    ...
    val updatedData = record.videoRewards.get(youTubeUpdate.videoId).map { data =>
      ...
      def updateYouTubeRewardInfoSameDay() = {
      ...
      if (isNewDay(data, currentEpochProgress)) updateYouTubeRewardInfoNewDay()
    ...
    val updatedDataSourceAddress = if (isNewDay(updatedData, currentEpochProgress)) {
      record
